...
public class ClientSocket
{
    ...
    public async Task<bool> MakeConnection(string ip, int port, string name)
    {
        PlayerName = name;
        try
        {
            ...
            string loginMsg = $"LOGIN {PlayerName} 0";
            await SendToServer(loginMsg);
            return true;
        }
        catch (Exception ex)
        {
            Debug.LogError($"[ClientSocket] 연결 실패: {ex}");
            return false;
        }
    }
